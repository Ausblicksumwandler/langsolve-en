...
This document gives a brief overview of the methods used to develop the solver. It is not intended to  comprehensively document them, but rather serves as an introduction of the main concepts and design choices that have been made, so as to facilitate other authors to later make their own contributions. In particular, two things are considered:
\begin{enumerate}
\item  Structure of the employed \LaTeX{} code 
\item Naming and usage conventions 
\end{enumerate}
Understanding both of these things is crucial for those willing  to contribute to any linguistic solution themselves. The resultant standardization  makes it possible to easily update and use the solver, irrespective of how complex and long it may become in the future. 

Although \bt{Git} and \bt{GitHub} are also necessary for  updating the real document, its  knowledge can be conveniently  obtained from the internet, and has been omitted.
...
\end{example}
\section{Conclusion}
Anyone willing to contribute to the solver must follow the following steps:
\begin{enumerate}
    \item Fork the repository \texttt{langsolve-en} on your GitHub account
    \item Edit the necessary source files
    \item Compile locally using LuaLaTeX, then debug possible errors and warnings
    \item Submit a Git pull request ONLY for the edited `.tex' files
\end{enumerate}
On reviewing the edit, the pull request may be granted, which then truly updates the solver.  A learner may duplicate this solver and comment out (using \%) all unnecessary languages in the main file, so as to reduce  total pdf length.
